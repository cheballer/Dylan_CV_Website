...
/*
 * GridScan — GLSL 3-D grid shader with scroll-reactive scan line.
 * Converted from TypeScript. face-api.js and postprocessing removed
 * (webcam/bloom not needed for a site background).
 * Only dependency: three (already in package.json).
 */
...
function smoothDampVec2(current, target, vel, smoothTime, dt) {
  ...
  const maxChange = 1e9 * smoothTime;
  ...
  const out = targetAdj.add(change.add(temp).multiplyScalar(exp));
...
export function GridScan({
  ...
  const skewScale  = lerpN(0.06, 0.2,  s);
  const tiltScale  = lerpN(0.12, 0.3,  s);
  const yawScale   = lerpN(0.1,  0.28, s);
  const smoothTime = lerpN(0.45, 0.12, s);
  const yBoost     = lerpN(1.2,  1.6,  s);
  ...
  useEffect(() => {
    ...
    const handler = (e) => {
      ...
      lookTarget.current.set(
        ...
        Math.max(-1, Math.min(1, -beta / 30)),
      );
      tiltTarget.current = (gamma * Math.PI / 180) * 0.4;
    };
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const lc = smoothDampVec2(lookCurrent.current, lookTarget.current, lookVel.current, smoothTime, dt);
      lookCurrent.current.copy(lc);

      const ts = smoothDampFloat(tiltCur.current, tiltTarget.current, tiltVel.current, smoothTime, dt);
      tiltCur.current = ts.value; tiltVel.current.v = ts.v;

      const ys = smoothDampFloat(yawCur.current, yawTarget.current, yawVel.current, smoothTime, dt);
      yawCur.current = ys.value; yawVel.current.v = ys.v;

      material.uniforms.uSkew.value.set(
        lookCurrent.current.x * skewScale,
        -lookCurrent.current.y * yBoost * skewScale,
      );
      material.uniforms.uTilt.value = tiltCur.current * tiltScale;
      material.uniforms.uYaw.value  = Math.max(-0.6, Math.min(0.6, yawCur.current * yawScale));
    ...
    sensitivity, lineThickness, linesColor, scanColor, scanOpacity,
    gridScale, lineStyle, lineJitter, scanDirection,
  ]);
